...
def create_fama_french_view(
    ...
    beta_results = Parallel(n_jobs=n_jobs)(
        delayed(estimate_factor_betas)(
            ...
            factor_returns_df,
            fx_returns_df.iloc[:, 0] if asset_fx_sensitivity.get(ticker, False) else None
        )
...
if __name__ == '__main__':
    ...
    mock_config = {'premium_estimator': 'long_term_mean', 'include_alpha': False}
    
    # --- WHEN ---
    betas, mu_view = create_fama_french_view(
        asset_returns,
        factors,
        FactorViewConfig(**mock_config),
